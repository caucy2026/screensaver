...
本次更新修复自动屏保选择与固件镜像兼容问题。已经观察到两屏共享帧、所选春之舞连续循环，以及 D0 触摸退出释放播放器。1.0.12 已验证 D2 触摸退出和正常系统屏保入口的双屏拼接；真实空闲计时和物理面板 24Hz 尚未独立验证。

`assets/` 包含可直接构建的最终媒体。`artwork/` 保留处理与设计记录，其中早期媒体处理脚本引用原外盘路径，属于历史记录；正常构建不需要重新生成媒体。

## S1 自动屏保兼容

启用时同步系统所选、默认及固件插电屏保组件。S1 固件正常屏保会强制添加 DreamMirror 镜像层；应用在 D0 启动时只做一次独立窗口接管，避免上屏被下屏镜像覆盖。验证时必须覆盖正常系统 dream 入口，不能仅测试立即运行。
